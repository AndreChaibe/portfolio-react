...
const NavigationHeader = () => {
  ...
  return (
    <nav
      className={`${theme.background === "bg-modal-white" ? "bg-white" : "bg-black"} rounded-2xl p-0 m-0 z-4 
                  max-[690px]:fixed 
                  max-[690px]:bottom-0 
                  max-[690px]:left-0 
                  max-[690px]:right-0
                  max-[690px]:rounded-[0]`}
    >
      <ul className="flex justify-center gap-8 py-3.5 px-7 max-[400px]:gap-6.5">
        {InforItems.map((item) => (
          <li
            key={item.route}
